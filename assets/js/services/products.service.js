...
export async function listarProdutos({ filtros = {}, ordenar = 'recentes', pagina = 1, porPagina = 12 } = {}) {
  ...
  if (filtros.precoMin != null && filtros.precoMin !== '') query = query.gte('preco', Number(filtros.precoMin));
  if (filtros.precoMax != null && filtros.precoMax !== '') query = query.lte('preco', Number(filtros.precoMax));
...
export async function listarOfertas(limite) {
  ...
  return (data || [])
    .filter(function(p) { return p.preco_promocional != null && Number(p.preco_promocional) < Number(p.preco); })
...
function normalizarProduto(p, completo) {
  ...
  const base = {
    ...
    temPromo: temPromo,
    descontoPct: temPromo ? Math.round((1 - Number(p.preco_promocional) / Number(p.preco)) * 100) : 0,
    condicao: p.condicao,
    estoque: p.estoque,
    status: p.status,
    destaque: !!p.destaque,
